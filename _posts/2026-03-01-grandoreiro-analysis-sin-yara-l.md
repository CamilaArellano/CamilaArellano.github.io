...
tags: [grandoreiro, yara, banking-trojan, ioc, mitre-attack, delphi, powershell, fileless, latam]
---
...
El **troyano bancario Grandoreiro** es una de las amenazas más activas y persistentes en el sector financiero y asegurador de Latinoamérica. Este proyecto desarrolló, dentro de un **Centro de Operaciones de Seguridad (SOC)**, mecanismos de detección basados en inteligencia de amenazas, incluyendo reglas **YARA** para el análisis de artefactos maliciosos. Las detecciones fueron validadas en entornos controlados sin generar falsos positivos, y la metodología es replicable para otras familias de malware similares.

---

## Abstract

The Grandoreiro banking Trojan represents one of the most significant threats to the financial and insurance sectors in Latin America due to its high capacity for evasion, persistence, and theft of sensitive information. This project was developed within the operational context of a Security Operations Center (SOC), applying threat analysis methodologies, collecting Indicators of Compromise (IOCs), and designing custom detection rules using YARA. The results demonstrated that the developed rules are specific, robust, and functional, without generating false positives during the evaluation period.
...
El desarrollo de este proyecto permitió robustecer las capacidades de detección y respuesta del SOC de una aseguradora mediante investigación de amenazas, recopilación de IOCs y desarrollo de reglas **YARA**.
...
- Crear reglas de detección basadas en **YARA** para identificar artefactos y patrones maliciosos asociados con Grandoreiro.
...
- **Host-based IOCs**: hashes de archivos (MD5, SHA-1, SHA-256), claves de registro y procesos sospechosos.
- **Network-based IOCs**: IPs, dominios C2, URLs y certificados digitales anómalos.
...
5. **Difusión** – Integración de los resultados en los procesos de seguridad.
...
El marco **MITRE ATT&CK®** documenta las tácticas, técnicas y procedimientos (TTPs) utilizados por actores de amenaza.
...
| 14 | Impacto | Alterar, destruir o cifrar sistemas o datos |
...
1. **Investigación del panorama de amenazas** – Estudio de reportes técnicos, feeds OSINT y campañas activas en LATAM.
...
4. **Desarrollo y validación de reglas de detección** – Creación de reglas YARA y pruebas en entorno controlado.
...
- Toma capturas de pantalla y las envía cifradas vía HTTPS.
...
| `$asm3`, `$asm4` | Reflexión | Obtención dinámica de tipos y métodos |
...
- Las reglas **YARA** detectaron correctamente los artefactos maliciosos evaluados sin falsos positivos en los archivos benignos utilizados durante las pruebas.
- Las reglas fueron validadas mediante pruebas controladas con muestras maliciosas y archivos benignos.

---

## 9. Conclusiones

El proyecto demostró que es posible desarrollar mecanismos de detección **específicos, robustos y operativamente viables** para el troyano bancario Grandoreiro dentro de un SOC corporativo, utilizando herramientas de inteligencia de amenazas y análisis técnico.
...
2. Las reglas **YARA** permitieron identificar artefactos asociados con campañas de phishing y loaders fileless mediante patrones técnicos específicos.
...
- **Automatizar la ingestión y validación de IOCs** desde fuentes OSINT y comerciales.
- **Reforzar controles en el vector phishing**, dado que sigue siendo uno de los principales mecanismos de distribución.
- **Implementar políticas de concienciación** para usuarios sobre la técnica *ClickFix* (portapapeles + PowerShell).
- Considerar actividades de **Threat Hunting** utilizando los IOCs y patrones técnicos documentados.
...
10. Kaspersky GReAT, "Grandoreiro, el troyano global con metas grandiosas". [https://securelist.lat/grandoreiro-banking-trojan](https://securelist.lat/grandoreiro-banking-trojan)
11. M. Frydrych et al., "IBM X-Force Threat Analysis: Hive0148 observed targeting Mexico and Costa Rica". [https://www.ibm.com/think/x-force/ibm-x-force-threat-analysis-hive0148-observed-targeting-mexico-costa-rica](https://www.ibm.com/think/x-force/ibm-x-force-threat-analysis-hive0148-observed-targeting-mexico-costa-rica)
